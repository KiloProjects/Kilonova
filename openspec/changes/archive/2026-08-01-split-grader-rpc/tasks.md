...
- [x] 6.2 Remote mode validated live against a real grader (localhost:9000, TLS+token) + platform (localhost:8070): platform connected and fetched the language inventory over RPC; submission #346 (cpp17, problem "sum") graded `finished` **100/100**, all 5 tests AC — full compile+execute over `RunBox3` + source/binary/test bytes over HTTP `/scratch`. Multibox (`RunMultibox3`) not driven by a live interactive submission (needs a problem-specific protocol) but shares the same conversion/scratch/dispatch plumbing, unit-covered by the lossless-conversion tests.
...
- [x] 6.4 Auth verified end-to-end over httptest: valid token served, missing/invalid token rejected with `Unauthenticated` **before any box executes** (`rpc_auth_test.go`). Also confirmed live: `Languages` RPC and `/scratch` PUT/GET/DELETE succeed with the real token and return 401 on a bad one; non-UUID / path-traversal ids rejected with 400 on the live grader.
