...
func (s *DB) ScoredProblem(ctx context.Context, problemID int, userID int) (*kilonova.ScoredProblem, error) {
	...
	err := Get(s.conn, ctx, &pb, `SELECT pbs.*, $2, COALESCE(ms.score, -1) AS score, (editors.user_id IS NOT NULL) AS pb_editor
FROM problems pbs 
...
func (s *DB) ScoredProblems(ctx context.Context, filter kilonova.ProblemFilter, userID int) ([]*kilonova.ScoredProblem, error) {
	...
	query := `SELECT problems.*, $1, COALESCE(ms.score, -1) AS score, (editors.user_id IS NOT NULL) AS pb_editor
FROM problems 
