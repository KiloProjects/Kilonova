...
func (a *DB) Attachments(ctx context.Context, filter kilonova.AttachmentFilter) ([]*kilonova.Attachment, error) {
	...
	query := a.conn.Rebind("SELECT " + selectedAttFields + " FROM attachments WHERE " + strings.Join(where, " AND ") + " ORDER BY name ASC " + FormatLimitOffset(filter.Limit, filter.Offset))
...
func (a *DB) ProblemAttachments(ctx context.Context, pbid int, filter *kilonova.AttachmentFilter) ([]*kilonova.Attachment, error) {
	var attachments []*dbAttachment
	where, args := attachmentFilterQuery(filter)
	query := a.conn.Rebind("SELECT " + selectedAttFields + " FROM attachments WHERE problem_id = $1 AND " + strings.Join(where, " AND ") + " ORDER BY name ASC " + FormatLimitOffset(filter.Limit, filter.Offset))
