...
func (s *DB) CreateTest(ctx context.Context, test *kilonova.Test) error {
	...
	var id int
	err := s.conn.GetContext(ctx, &id, "INSERT INTO tests (score, problem_id, visible_id, orphaned) VALUES ($1, $2, $3, $4) RETURNING id", test.Score, test.ProblemID, test.VisibleID, test.Orphaned)
	if err == nil {
		test.ID = id
	}
	return err
...
func (s *DB) OrphanProblemTests(ctx context.Context, problemID int) error {
	_, err := s.pgconn.Exec(ctx, "UPDATE tests SET orphaned = true WHERE problem_id = $1", problemID)
	if err != nil {
		return err
	}
	_, err = s.pgconn.Exec(ctx, "DELETE FROM subtask_tests WHERE test_id IN (SELECT id FROM tests WHERE problem_id = $1)", problemID)
	return err
}

func (s *DB) OrphanTest(ctx context.Context, id int) error {
	_, err := s.pgconn.Exec(ctx, "UPDATE tests SET orphaned = true WHERE id = $1", id)
	if err != nil {
		return err
	}
	_, err = s.pgconn.Exec(ctx, "DELETE FROM subtask_tests WHERE test_id = $1", id)
	return err
}

func (s *DB) BiggestVID(ctx context.Context, problemID int) (int, error) {
	var id int
	err := s.pgconn.QueryRow(ctx, "SELECT visible_id FROM tests WHERE problem_id = $1 AND orphaned = false ORDER BY visible_id DESC LIMIT 1", problemID).Scan(&id)
	return id, err
}
