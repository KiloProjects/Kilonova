...
- `./kn main` runs the platform; `./kn grader-serve` runs a standalone remote grader. Config: `config.toml` (`-c`, static) + `flags.json` (`-f`, runtime-editable) + `grader.toml` for the remote grader; all are gitignored, `*.example.toml` are the templates.
...
- **`domain/`** — newer, more isolated pieces: `config` (TOML config + flag registry), `datastore` (afero-backed buckets for tests/subtests/attachments/avatars), `user`, `archive` (problem import/export).
- **Configuration & flags** — `config.toml` holds startup-only settings (`config.Common`, `config.Eval`, …). Everything runtime-tunable is a flag declared with `config.GenFlag[T]("dotted.key", default, description)` in `sudoapi/flags/{backend,frontend}_flags.go`, persisted to `flags.json`, and editable from the admin UI. Add new toggles as flags, not config fields; gate routes with `rt.checkFlag(...)`.
- **Grading** — `eval/grader` polls the DB for waiting submissions (`feeder.go`) and evaluates them (`grader.go`) through an `eval.BoxScheduler`. Two modes, chosen by `[eval] mode` in config.toml: *local* runs `eval/scheduler` in-process over `eval/box` (isolate sandboxes; `stupidbox` is the insecure fallback, blocked unless `feature.grader.force_secure_sandbox` is off), *remote* talks JSON over HTTP to a separate `kn grader-serve` process (`eval/scheduler/rpc_{client,server}.go`; `POST /run/box3`, `POST /run/multibox3`, `GET /languages`; file transfer via `eval/scratch` on `/scratch/{id}`; platform and grader must be the same build). Language definitions and version probing live in `eval/language` + `scheduler/langmgr.go`.
- **Assets** — `web/assets/vite.config.ts` runs one build per `BUNDLE` (`bundled` = app.ts, `vendored` = vendored.ts, `css`), because templates call IIFE globals from inline scripts. `web/assets.go` merges the per-bundle manifests at init and `assets.Asset("app.ts")` resolves the hashed `/static/misc/...` URL. `web/assets_test.go` verifies the manifests point at files that actually got embedded — it fails if the asset build was skipped.
