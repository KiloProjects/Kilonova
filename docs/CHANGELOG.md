...
First release under the new `YY.MM` versioning scheme. Everything since v0.25.2.
...
- Datastore rewritten on top of afero; stored files are no longer compressed.

### Remote grader

- The sandbox executor can run as a separate `kn grader-serve` process. The platform talks JSON over HTTPS
  (`POST /run/box3`, `POST /run/multibox3`, `GET /languages`) with a per-client bearer token registry, and transfers
  files through the grader's `/scratch` endpoint. The earlier ConnectRPC/protobuf and SFTP experiments were removed
  before release.
- Language management split out of the box scheduler; box processing sits behind an abstract interface so local and
  remote runners share the grading code.

### Grading & languages

- Communication (interactive) problem type, with the checker renamed to *manager* and manager/stub attachment values
  autofilled.
- Rudimentary AI task type: submissions graded by an LLM checker running under `uv`, with AI-generated uploads.
- Full Java support, including multi-class programs. C++23 added.
- Memory limit exceeded is now reported properly; checker memory limit raised; Python batch grading improved for
  low-memory tasks; communication task time limits fixed; grader language version detection fixed.
- Compile and execute tasks are independent of submissions/problems.
- Multiple files per submission (#86), with the submitter's IP address recorded.
- Test archive parsing override for problem archives; autofilled test scores now round up and end in ascending order
  (#61).
- Test and subtask editing uses internal IDs (#66).

### Contests

- Per-contest IP whitelist with management toggles.
- QR codes for contest invites; clone contest button; contestant query parameter on contest pages.
- Fixes: users in virtual contests can see their own submissions; running contests shown correctly to unauthenticated
  visitors; ICPC submission display; contest announcement context-cancel errors.

### Problems & editing

- Bulk problem import.
- Problem review request column.
- German added as a statement language.
- Problem lockdown for published problems is configurable (flag).
- Flag to hide the statistics tab.
- Experimental vim mode in the Markdown attachment editor; `noTransparency` option for images.
- Internal submission export.
- Fixes: problem edit page, PDF downloads, checker stem handling, multipart form for adding files.

### Auth & security

- Kilonova is now an OpenID Connect provider (OAuth grant UI, `/oauth` flows) and the v2 API accepts OAuth security
  schemes.
- Session cookie is always `Secure`; all client cookies set `secure`; CSRF protection on logout (#67, #71).
- CAPTCHA on the forgot-password form and on problem archive downloads; asset downloads are served as attachments.
- Better support for Scaleway TEM as the SMTP provider.

### API v2

- Re-enabled with OpenAPI improvements and CORS.
- New endpoints: get problem by ID (with tags, languages and statement variants), get languages, create submission, get
  submission, get contest, statement variants. Problem list moved from GET to POST.

### Web UI

- Many pages migrated from `html/template` to templ: submissions, users, lockout, contest pages, problem list, problem
  info and contest disclaimer, submission and paste pages (now server-side rendered), plus a server-rendered pagination
  component.
- Dedicated per-user submissions page; trending problems tab optimized.
- Navbar and footer styling refresh; login modal fixes; `server-timestamp` custom element replaces the
  `server_timestamp` class; CodeMirror logic abstracted.
- Fixes: submissions page checkbox, submission timestamp formatting, hljs copy button colouring on theme change,
  unverified email warning, auto-submit of accepted solutions form, max memory display on the submissions page, older
  submissions showing the correct user, 404 handler after the OIDC reorganisation.

### Internals

- Configuration moved out of `internal/` into `domain/config` with all flag definitions in `sudoapi/flags`.
- Postgres connection, user context and user repository extracted into their own packages; avatar handling moved to
  `web`.
- Remaining `zap` logging replaced with `slog`; large golangci-lint cleanup.
- OpenSpec adopted for spec-driven changes (`openspec/`).
- Docs website started (mkdocs), with a page on problem statements.
...
- List of changes by month:
    - May
        ...
        - !!! Huge refactoring of task execution, paving the way for remote graders and, finally, an official v1.0
    ...
    - April
        ...
        - Add eviction policies based on TTL and max file size for bucket files (in cache environments such as
          subtests);
    ...
    - March
        ...
        - Changed the visibility requirements for problems to be added to virtual contests;
            - For example, a problem from an official contest could, in the past, be added to a virtual contest and
              bypass the submission count limit.
        ...
        - Huge performance improvements to calculating problem score by caching and recalculating on submission changes
          instead of storing in a view;
    ...
    - February
        ...
        - Save session information such as user agent and ip, used mostly for official contests to protect against
          cheating.
    - January
        ...
        - Score "scaling" feature (for example, you can have 1000 points show up in the contest leaderboard for a
          problem);
        - Output only problem handling improvements;
            - Variable max submission size;
            - Default to file upload regardless of preference (avoids nasty lag when copy-pasting large text in
              codemirror).
...
- Markdown statements now require no browser javascript for proper rendering (CSS is required, though, for KaTeX
  expressions);
...
- Add buttons for quick toggling problem visibility on problem lists (only shallow visibility toggle, it doesn't
  recurse).
