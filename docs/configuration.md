...
`common.log_dir` and `grader.scratch_dir` have no replacement: both processes use `$KN_DATA_DIR/logs`, and the grader uses `$KN_DATA_DIR/scratch`. Move or symlink your old log directory if you want the history in one place.
